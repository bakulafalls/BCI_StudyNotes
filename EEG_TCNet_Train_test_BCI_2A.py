...
def buttferfiter(data):
    Fs = 250
    b, a = signal.butter(4, [7, 47], 'bandpass',fs=Fs)  # 根据文献建议选择滤波器参数
...
class feature_dataset(Dataset):
    ...
    def parse_data_file(self,file_path):   
        data = pd.read_csv(file_path,header=None)  # 288, 22000
        ...
        data = data.reshape(num_sub,22,1000)  # 288, 22, 1000
        data = torch.tensor(data)
        # 第一层CNN输入通道为1，扩一维
        data = data.unsqueeze(1)  # 288, 1, 22, 1000
        '''
        for row in data:
          row = row.strip(" ").split(' ')
...
train_dataset = feature_dataset(file_path = r'D:\work\data\BCICIV_2a_EEGNet\EEG-TCNet\MI-EEG-A01T.csv',
                                target_path=r'D:\work\data\BCICIV_2a_EEGNet\EEG-TCNet\EtiquetasA01T.csv')
train_dataloader = DataLoader(train_dataset,shuffle=True,batch_size=config.batch_size)

val_transforms = transforms.Compose([transforms.ToTensor()])
val_dataset = feature_dataset(file_path =r'D:\work\data\BCICIV_2a_EEGNet\EEG-TCNet\MI-EEG-A01E.csv',
                               target_path=r'D:\work\data\BCICIV_2a_EEGNet\EEG-TCNet\EtiquetasA01E.csv')
val_dataloader = DataLoader(val_dataset,batch_size=config.batch_size)  
set_seed()

model = EEG_TCNet().to(device)
# SGD优化器, 学习率lr=0.01, 衰减率0.0001
optimizer = optim.SGD(model.parameters(),lr=config.lr,weight_decay=1e-4)
# 交叉熵
criterion = torch.nn.CrossEntropyLoss()
loss_fn = criterion.to(device)   # 损失函数放在GPU上
...
def DrawConfusionMatrix(save_model_name,val_dataloader):
    ...
    disp.plot()
    save_hunxiao_path = os.path.join(save_model_name,'confusion_matrix.png')
...
DrawConfusionMatrix(r"D:\work\data\BCICIV_2a_EEGNet\EEG-TCNet",val_dataloader)
